...
class ANCSNotification {
     ...
     */
    constructor(id, title, subtitle, body, appID, date) {
        this.id = id;
        this.title = title;
...
class ANCSClient extends EventEmitter {
     ...
     */
    #extractMac(path) {
        const match = path.match(/dev_([0-9A-F_]+)/);
    ...
    async isSupported() {
        ...
        try {
            ...
            let bluez = await this.bus.getProxyObject("org.bluez", "/");
            ...
            for (const path of Object.keys(objects).filter((v) => this.#extractMac(v) === this.MACAddress)) {
                let uuid = objects[path].UUID.value.toUpperCase();
    ...
    async startListening() {
        ...
        for (const path of Object.keys(objects).filter((v) => this.#extractMac(v) === this.MACAddress)) {
            let char = objects[path].interfaces["org.bluez.GattCharacteristic1"];
            if(!char || !char.UUID) continue;
        ...
        let notifObj = await this.bus.getProxyObject('org.bluez', this.#notificationSource.path);
        ...
        const obj = await this.bus.getProxyObject("org.bluez", this.controlPoint.path);
        ...
        this.#notificationSource.listener = notifProps.on('PropertiesChanged', async (iface, changed) => {
            ...
            if (changed.Value?.value) {
                ...
                if(data.substring(0, 2) == "02") {
                    ...
                    */
                    this.emit("removed", this.notifications[data.substring(data.length - 8)]);
        ...
        const dataObj = await this.bus.getProxyObject("org.bluez", this.dataSource.path);
        ...
        this.#dataSource.listener = dataProps.on("PropertiesChanged", async (iface, changed) => {
            ...
            if(changed.Value?.value) {
                ...
                if (data[0] == 0) {
                    ...
                    let notificationobj = new ANCSNotification(notificationID, title, subtitle, body, appID, date);
                    if(!this.notificatis[notificationID]) {
